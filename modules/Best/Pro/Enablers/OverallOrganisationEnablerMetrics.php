...
abstract class OverallOrganisationEnablerMetrics
{
                        ...
                        if ($techAvg > self::COMMENT_POINT_8_LEFT && $workflowAvg > self::COMMENT_POINT_4_LEFT) {
                            ...
                            if ($techAvg < self::COMMENT_POINT_4_TOP && $workflowAvg > sefl::COMMENT_POINT_4_LEFT) {
                                if ($workflowAvg > self::COMMENT_POINT_8_LEFT && $techAvg < self::COMMENT_POINT_8_TOP) {
                                    ...
                                    $comment = trans("The 'technology' component score seems to suggest a need for significant improvements which :name can work on for better efficiency. || Given the presence of clear 'processes', better implementation of a more robust technology can help improve overall efficiency.", ['name' => $name]);
                                }
                            } else {
                                $comment = trans(":name has shown positive adoption of technology into its work flow. || Significant improvements can still be achieved if the non-technology enablers, such as talent mix, are reviewed together to further enhance on the organisational efficiency.", ['name' => $name]);
                            }
                        }
                    }
                }
    ...
    public static function getSecondComment($docuAvg, $talentAvg, $workflowAvg, $name)
    {
        ...
        if ($workflowAvg < self::COMMENT2_POINT_4_PROCESS && $docuAvg < self::COMMENT2_POINT_4_DOCU && $talentAvg < self::COMMENT2_POINT_4_TALENT) {
            $comment = trans("A thorough review of the 'documentation' practices and processes should be carried out while process gaps within the HR practices relating to 'talent' development are being conducted.");
        } else {
            if ($workflowAvg < self::COMMENT2_POINT_4_PROCESS && $docuAvg < self::COMMENT2_POINT_4_DOCU && $talentAvg > self::COMMENT2_POINT_4_TALENT) {
                ...
                if ($workflowAvg < self::COMMENT2_POINT_4_PROCESS && $docuAvg < self::COMMENT2_POINT_4_DOCU && $talentAvg < self::COMMENT2_POINT_4_TALENT) {
                    ...
                    $comment = trans("Given that considerable 'documentation' and clear talent recruitment and development practices are already in place, periodic efficiency gaps review in these areas is still recommended. || This is to ensure that complacency does not seep in to keep everyone in check.");
                }
            }
        }
    ...
    public static function getWorkflowProcessComment($workflowAvg, $name)
    {
        ...
        if ($workflowAvg < config('modules.best.scores.grades.red')) {
            if ($workflowAvg < config('modules.best.scores.grades.nonlight')) {
                $comment = trans(":name should consider performing an in-depth review of all work processes across all organisational areas to ensure a sound work structure is set up. Growth will be slow if work structure does not evolve with the changing needs.", ['name' => $name]);
            } else {
                $comment = trans("Organisation has shown some basic adoption of work processes. More can still be done to ensure that effective monitoring and governance of the expectations are practiced organisation-wide.");
            }
        } else {
            if ($workflowAvg > config('modules.best.scores.grades.amber')) {
                $comment = trans("Overall, :name has indicated very good adoption of effective and efficient workflow processes that are well practiced. Such practices and culture should continue to be encouraged to maintain organisational efficiency.", ['name' => $name]);
            } else {
                $comment = trans("Overall, :name's workflow processes have met the minimum expectations.", ['name' => $name]);
            }
        }
    ...
    public static function getTalentComment($talentAvg)
    {
        ...
        if ($talentAvg < config('modules.best.scores.grades.red')) {
            if ($talentAvg < config('modules.best.scores.grades.nonlight')) {
                $comment = trans("Organisation has to seriously support & develop employees in preparing them to perform & contribute to the organisational operations. Ignoring the role played by employees may significantly impact organisational effectiveness.");
            } else {
                $comment = trans("Organisation has show some basic processes in place but more can still be done to ensure that sufficient employee development and employee involvement is implemented.");
            }
        } else {
            if ($talentAvg > config('modules.best.scores.grades.amber')) {
                $comment = trans("Overall, very good employee development is in place where employee involvement is highly encouraged across the organisation.");
            } else {
                $comment = trans("Overall, the score seemed to suggest that sufficient attention have been given to employees in ensuring they are able to contribute to the organisational objectives");
            }
        }
    ...
    public static function getDocumentationComment($documentationAvg)
    {
        ...
        if ($documentationAvg < config('modules.best.scores.grades.red')) {
            if ($documentationAvg < config('modules.best.scores.grades.nonlight')) {
                $comment = trans("Detailed review of documentation practices and policies are required. This may have serious ramifications when audits needs to be carried, internally or externally.");
            } else {
                $comment = trans("While some documentation processes are observed, more can still be done to ensure that these expectations are clearly communicated to employees on how it should be carried out.");
            }
        } else {
            if ($documentationAvg > config('modules.best.scores.grades.amber')) {
                $comment = trans("Overall, documentation processes have have exceeded expectations.");
            } else {
                $comment = trans("Overall, the documentation processes and adoption across the organisation have met the minimum expectations.");
            }
        }
    ...
    public static function getTechnologyComment($technologyAvg, $name)
    {
        ...
        if ($technologyAvg < config('modules.best.scores.grades.red')) {
            if ($technologyAvg < config('modules.best.scores.grades.nonlight')) {
                $comment = trans("A comprehensive ICT needs analysis is strongly advised given the minimal technology utilisation. Rapid technology advancement may leave organisation far behind if not adopted appropriately.");
            } else {
                $comment = trans("While some use of technology is in place, more needs to be done to ensure that the processes maximises the available systems and tools available to achieve greater work efficiency.");
            }
        } else {
            if ($technologyAvg > config('modules.best.scores.grades.amber')) {
                $comment = trans("Overall, substantial technology adoption is present that should drive the organisational efficiency and facilitate streamlining of work processes.");
            } else {
                $comment = trans("Overall, :name has shown efforts to integrate and adopt relevant technology to enhance their work processes.", ['name' => $name]);
            }
        }

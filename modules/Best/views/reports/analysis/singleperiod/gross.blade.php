...
<section>
  <div class="row">
    <div class="col-md-12">
      <div>
        <h1 class="mb-3 dt-secondary">@lang('Gross Profit Score')</h1>
      </div>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-md-12 chart-analysis">
      <div class="card mb-5">
        <div class="card-body">
          <canvas height="100" id="gross_ratio"></canvas>
        </div>
      </div>
    </div>
    <div class="col-md-12 comment-analysis">
        <div class="row">
          ...
          <div class="col">
              <p>{{ $data['analysis:financial']['gross_ratio']['comment'][0] }}</p>
          </div>
        </div>
    </div>
  </div>
...
$(document).ready(function() {
  ...
  var labels = {!!  json_encode(collect($data['analysis:financial']['gross_ratio']['chart']['labels']['preview'])->values()->toArray()); !!}
  var barChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: labels,
      // labels: ["Gross Profit Margin", "Recommended Good Score"],
      datasets: dataset,
    },
    options: {
      cornerRadius: 0,

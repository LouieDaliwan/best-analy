...
class Solvency {
    ...
    public static function compute($solvency, $balanceSheets)
    {
        ...
        round( ((float) $balanceSheets['total_liabilities'] / (float) $balanceSheets['stockholdersequity']), 3) : 0;

        $solvency['debt_ratio'] = (float) $balanceSheets['total_assets'] != 0 ? round(
                ((float) $balanceSheets['total_liabilities'] / (float) $balanceSheets['total_assets']), 3
            ) 
            : 0;

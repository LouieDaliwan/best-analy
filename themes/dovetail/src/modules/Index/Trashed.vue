<template>
  <admin>
    <metatag :title="trans('Trashed Components')"></metatag>

    <page-header
      :back="{ to: { name: 'indices.index' }, text: trans('Components') }"
    ></page-header>
        ...
        <v-slide-y-reverse-transition mode="out-in">
          ...
          >
            ...
            <template v-slot:item.name="{ item }">
              <span class="muted--text" :title="item.name">{{
                trans(item.name)
              }}</span>
            </template>
            <!-- Name -->
            ...
            <template v-slot:item.description="{ item }">
              ...
              >
                <template v-slot:activator="{ on }">
                  <span v-on="on" class="text--ellipsis-1 muted--text">{{
            ...
            <template v-slot:item.deleted_at="{ item }">
              <span class="muted--text" :title="item.deleted_at">{{
                trans(item.deleted)
              }}</span>
            </template>
            <!-- Deleted -->
            ...
            <template v-slot:item.action="{ item }">
              <div class="text-no-wrap">
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn @click="restoreResource(item)" icon v-on="on">
                      <v-icon class="mdi-spin" small v-if="item.loading"
                        >mdi-loading</v-icon
                      >
                ...
                <v-tooltip bottom>
                  ...
                  <span>{{
                    trans_choice("Permanently delete this index", 1)
                  }}</span>
                </v-tooltip>
                <!-- Permanently Delete -->
              </div>
    ...
    <div v-if="resourcesIsEmpty">
      <empty-state>
        <template v-slot:actions>
          <v-btn large color="primary" exact :to="{ name: 'indices.index' }">
            {{ trans("Go back to all Component") }}
          </v-btn>
        </template>
      </empty-state>
    </div>
...
export default {
  ...
  methods: {
    ...
    bulkRestoreResources() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.showSnackbar({
            text: trans_choice(
              "Component successfully restored",
    ...
    restoreResource(item) {
      item.loading = true;
      axios.patch($api.restore(item.id)).then(response => {
        this.getPaginatedData(null);
        this.showSnackbar({
          text: trans_choice("Component successfully restored", 1)
        });
      });
    },
    ...
    askUserToPermanentlyDeleteResource(item) {
      this.showDialog({
        ...
        text: [
          "Some data related to the account will still remain.",
          trans("Are you sure you want to permanently delete?", {
            name: item.name
          })
        ],
        buttons: {
    ...
    deleteResource(item) {
      ...
      axios
        ...
        .then(response => {
          ...
          this.hideDialog();
          this.showSnackbar({
            text: trans_choice("Component successfully deleted", 1)
          });
        })
        .catch(err => {
          this.errorDialog({

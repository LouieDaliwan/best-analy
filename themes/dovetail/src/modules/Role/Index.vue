              ...
              <template v-slot:item.name="{ item }">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <span class="mt-1" v-on="on"><router-link tag="a" exact :to="goToShowRolePage(item)" v-text="item.name" class="text-no-wrap text--decoration-none"></router-link></span>
                  </template>
...
export default {
  ...
  methods: {
    ...
    goToShowRolePage (role) {
      return { name: 'roles.show', params: { id: role.id, slug: role.rolename } }
    },
